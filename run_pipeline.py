...
def scrape_jobs_with_jobspy(
    ...
    try:
        ...
        if jobs_df is None or jobs_df.empty:
            log.warning("Jobspy scraping returned no results or failed."); return None
        else:
            ...
            return jobs_df
    except ImportError as ie: log.critical(f"Import error during scraping: {ie}. Ensure all jobspy dependencies are installed."); return None
        ...
        log.info(f"Saved {len(jobs_list)} scraped jobs to {output_path}"); return jobs_list
         ...
         for i, record in enumerate(jobs_list):
              try: json.dumps(record)
                   ...
                   break
         return []
    except Exception as e: log.error(f"Error saving scraped jobs to JSON file {output_path}: {e}", exc_info=True); return []
...
def run_pipeline():
    ...
    try:
        ...
        if args.filter_proximity_location and args.filter_proximity_range is None:
             parser.error("--filter-proximity-range is required with --filter-proximity-location.")
        ...
        if not structured_resume: log.critical("Failed to load and extract data from resume.", exc_info=True); sys.exit(1)
        ...
        print(); log.warning("[yellow]Pipeline execution interrupted by user (Ctrl+C). Exiting gracefully.[/yellow]")
        sys.exit(130)
