...
# Removed lru_cache import, using manual dict cache
...
GEOCODER = None
if GEOPY_AVAILABLE:
    # Use user agent from config.py
    import config # Import config here for user agent
    if not config.GEOPY_USER_AGENT or 'example.com' in config.GEOPY_USER_AGENT:
         log.warning("GEOPY_USER_AGENT in config.py is not set or uses placeholder. Geocoding might fail. Set a unique user agent.")
    ...
    try:
        ...
        if location_data and location_data.latitude and location_data.longitude:
            ...
            if country_code and not country_name:
                 cc_map = {'us': 'United States', 'usa': 'United States', 'ca': 'Canada', 'gb': 'United Kingdom', 'uk': 'United Kingdom', 'de': 'Germany', 'fr': 'France', 'au': 'Australia'} # Add more as needed
...
def apply_filters(
    ...
    # (Function content remains the same as previous version - no async changes needed here)
    # ... (Keep the detailed filtering logic from the version that included geocoding) ...
    filtered_jobs = []
    ...
    for job in jobs:
        ...
        if normalized_work_models:
             ...
             if not job_model: job_loc_wm = normalize_string(job.get('location'));
             if 'remote' in job_loc_wm: job_model = 'remote'; elif 'hybrid' in job_loc_wm: job_model = 'hybrid'; elif 'on-site' in job_loc_wm or 'office' in job_loc_wm: job_model = 'on-site'
             if not job_model or job_model not in normalized_work_models: passes_all_filters = False; log.debug(f"FILTERED (Work Model): '{job_title}' ({job_url}). Job '{job_model}' not in {normalized_work_models}"); continue
        ...
        if passes_all_filters and normalized_remote_country:
             ...
             if not passes_all_filters: continue # Exit early if failed
